...
class SenecaClient {
    constructor() {
        this._opened = false;
        this._schemas = {};
        this._dependencyResolver = new pip_services_commons_node_5.DependencyResolver(SenecaClient._defaultConfig);
    ...
    call(role, cmd, correlationId, params = {}, callback) {
        let err = null;
        if (role == '')
            err = new pip_services_commons_node_4.UnknownException(null, 'NO_ROLE', 'Missing Seneca pattern role');
        if (cmd == '')
            err = new pip_services_commons_node_4.UnknownException(null, 'NO_COMMAND', 'Missing Seneca pattern cmd');
        if (err) {
            if (callback)
                callback(err, null);
            else
        ...
        params.role = role;
        params.cmd = cmd;
        correlationId = correlationId || pip_services_commons_node_2.IdGenerator.nextShort();
        params.correlation_id = correlationId;
        this._seneca.act(params, callback);
    }
}
SenecaClient._defaultConfig = pip_services_commons_node_1.ConfigParams.fromTuples("dependencies.seneca", "pip-services-seneca:seneca:instance:*:*", "connection.protocol", "http", "connection.host", "0.0.0.0", "connection.port", 3000, "options.connect_timeout", 30000);
